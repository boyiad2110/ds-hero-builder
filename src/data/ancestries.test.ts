...
import { ANCESTRIES, ANCESTRY_SOURCE } from './ancestries'
...
describe('ancestry canonical identities', () => {
  it('matches the complete frozen Heroes 1.01b identity set', () => {
    expect(ANCESTRIES).toEqual(EXPECTED_ANCESTRIES)
    expect(ANCESTRIES).toHaveLength(12)
  })

  it('contains no duplicate canonical IDs', () => {
    const ids = ANCESTRIES.map(({ id }) => id)

    expect(new Set(ids).size).toBe(ids.length)
  })

  it('traces the records to the pinned upstream revision', () => {
    expect(ANCESTRY_SOURCE).toEqual({
